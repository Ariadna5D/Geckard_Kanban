...
const PublicRoute = ({ children }: { children: React.ReactNode }) => {
  ...
  // Espera a que el store recupere sesion antes de decidir ruta publica
  if (!hasHydrated) {
    return null;
  }
...
function RootRedirect() {
  ...
  // Evita redireccion temprana hasta que termine la hidratacion
  if (!hasHydrated) {
    return null;
  }
...
export const App = () => {
  const { _hasHydrated, fetchUser, token, setHasHydrated } = useAuthStore();

  useEffect(() => {
    // Si hay token recuperado vuelve a pedir perfil actual
  ...
  useEffect(() => {
    // Fallback simple por si la hidratacion queda colgada
