...
type TaskDetailSprintSectionProps = {
  ...
  /** Whether this task counts as part of the active sprint. */
  inActiveSprint: boolean;
  onInActiveSprintChange: (nextValue: boolean) => void;
};

/**
 * Lets editors attach the task to the current sprint (or remove it) while a sprint is running.
 */
    ...
    <TaskDetailSection
      ...
      titleAccessory={
        <TaskDetailInfoTip
          ...
          side="right"
          text={`Sprint activo en el tablero: «${activeSprintName}». Si marcas la casilla, esta tarea cuenta en ese sprint (vista filtrada del tablero y cierre del sprint).`}
        />
      }
    >
      <div className="space-y-3">
        ...
        <label className="flex cursor-pointer items-start gap-2 text-sm text-surface-800 dark:text-surface-100">
          <input
            type="checkbox"
            className="mt-0.5 size-4 rounded border-surface-400"
            disabled={readOnly}
