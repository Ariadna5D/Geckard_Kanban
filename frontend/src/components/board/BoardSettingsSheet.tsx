...
// Limita la carga de actividad
const BOARD_ACTIVITY_MAX_REQUEST = 200;

// Traduce el rol a texto visible
function roleLabel(role: string): string {
  if (role === "owner") return "Propietario";
  for (let roleIndex = 0; roleIndex < MANAGEABLE_ROLES.length; roleIndex++) {
    if (MANAGEABLE_ROLES[roleIndex].value === role) {
      return MANAGEABLE_ROLES[roleIndex].label;
    }
  }
  return role;
}
...
function apiErr(error: unknown): string {
  if (isAxiosError(error)) {
    const errorBody = error.response?.data as { message?: string | string[] };
...
export function BoardSettingsSheet({
  ...
  const inviteMember = useActiveBoardStore((state) => state.inviteMember);
  const removeBoardMember = useActiveBoardStore(
    (state) => state.removeBoardMember,
  );
  ...
  const loadMembers = useCallback(async function loadMembers(
    ...
    try {
      // Pedimos miembros al back y guardamos rol actual para cada fila
      const boardMembersResponse = await getBoardMembersRequest(boardDocId);
      setListError(null);
      setOwnerId(boardMembersResponse.ownerId);
      setMembers(boardMembersResponse.members);
      const drafts: Record<string, BoardInviteRole> = {};
      const membersList = boardMembersResponse.members;
      ...
      if (spin) {
        setListError("No se pudo cargar la lista de participantes.");
      } else {
        setListError("No se pudo actualizar la lista.");
      }
    } finally {
  ...
  const handleSaveBoard = async () => {
    ...
    setSavingBoard(true);
    try {
      // Guardamos cambios basicos del tablero en la api
      await updateBoardRequest(boardDocId, {
        title: editTitle.trim(),
        description: editDescription.trim() || undefined,
      });
      // Recargamos tablero para reflejar datos nuevos en pantalla
      await useActiveBoardStore.getState().fetchBoard(slug, { silent: true });
      setPanel("menu");
    } catch (errorUnknown) {
      setListError(apiErr(errorUnknown));
    } finally {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setSheetDangerError(apiErr(errorUnknown));
    } finally {
      setDeleting(false);
      setDeleteOpen(false);
    }
  ...
  const handleUpdateRole = async (memberUserId: string) => {
    ...
    for (let memberIndex = 0; memberIndex < members.length; memberIndex++) {
      if (members[memberIndex].userId === memberUserId) {
    ...
    try {
      ...
      setMembers(function replaceMemberRole(prev) {
        ...
        for (let memberIndex = 0; memberIndex < prev.length; memberIndex++) {
  ...
  const handleConfirmExpel = async () => {
    ...
    try {
      ...
      setMembers(function withoutExpelledMember(prev) {
        const out: BoardMemberSummary[] = [];
        for (let memberIndex = 0; memberIndex < prev.length; memberIndex++) {
          if (prev[memberIndex].userId !== memberUserId) {
            out.push(prev[memberIndex]);
          }
        }
        return out;
      });
  ...
  async function handleOpenActivityPanel() {
    ...
    try {
      // Cargamos actividad con limite para no traer hisotrial enorme
      await loadBoardActivity(boardDocId, BOARD_ACTIVITY_MAX_REQUEST);
    } catch (errorUnknown) {
  ...
  async function handleSprintsEnabledChange(nextEnabled: boolean) {
    ...
    setSprintsPanelError(null);
    try {
      // Activamos o desactivamos sprints para este tablero
      await updateBoardRequest(boardDocId, {
  ...
  async function handleSaveActiveSprintSettings() {
    ...
    try {
      ...
      // Mandamos cambios del sprint activo al store para guardar en back
  ...
  async function handleSaveClosedSprintName(sprintId: string) {
    ...
    try {
      ...
      await fetchBoard(slug, { silent: true });
      setEditingClosedSprintId((currentEditingSprintId) => {
        if (currentEditingSprintId === sprintId) {
          return null;
        }
        return currentEditingSprintId;
      });
    } catch (errorUnknown) {
      setSprintsPanelError(apiErr(errorUnknown));
    } finally {
  ...
  const archivedQueryLower = archivedQuery.trim().toLowerCase();
  const filteredArchivedTasks = archivedTasks.filter((taskRow) => {
    if (archivedQueryLower.length === 0) {
      return true;
    }
    return taskRow.title.toLowerCase().includes(archivedQueryLower);
  });
  ...
  return (
    <>
      ...
      >
        <SheetContent
          className="flex min-h-0 w-11/12 flex-col gap-0 overflow-hidden border-l border-surface-200 bg-surface-50 p-0 sm:max-w-4xl dark:border-surface-800 dark:bg-surface-900"
          showCloseButton
        >
          <SheetHeader className="shrink-0 space-y-0 border-b border-surface-200 p-0 dark:border-surface-800">
            <div className="flex items-center gap-2 pr-12 pl-4 pt-4 sm:pl-6 sm:pt-6">
              {panel !== "menu" && (
                <Button
                  type="button"
                  variant="ghost"
              ...
              <SheetTitle className="flex-1 py-1 text-left text-2xl font-semibold text-surface-900 dark:text-surface-50">
            ...
            {panel === "menu" && (
              <SheetDescription className="px-4 pb-4 text-left text-base text-muted-foreground sm:px-6 sm:pb-6">
          ...
          {panel === "menu" && (
            <div className="flex flex-1 flex-col gap-3 overflow-y-auto bg-surface-100 p-4 text-base sm:p-6 dark:bg-surface-950">
              {sheetDangerError !== '' && (
              ...
              {!canSettings && !canDelete && (
                <p className="text-muted-foreground mb-1 text-base leading-relaxed">
              ...
              {canSettings && (
                <Button
                  type="button"
                  variant="outline"
                  className="h-auto w-full justify-start gap-3 border-surface-200 bg-surface-50 py-4 text-lg font-semibold text-surface-800 dark:border-surface-700 dark:bg-surface-900 dark:text-surface-100"
                  onClick={handleOpenEditPanel}
                >
                  <Pencil className="size-5 shrink-0 opacity-80" />
              ...
              {canSettings && (
                <Button
                  type="button"
                  variant="outline"
                  className="h-auto w-full justify-start gap-3 border-primary-500/40 bg-primary-500/10 py-4 text-lg font-semibold text-primary-900 hover:bg-primary-500/15 dark:border-primary-400/40 dark:bg-primary-500/20 dark:text-primary-100 dark:hover:bg-primary-500/30"
                  onClick={handleOpenSprintsPanel}
                >
                  <Flag className="size-5 shrink-0 opacity-90" />
              ...
              {canLeaveBoard && (
                ...
                >
                  <LogOut className="size-5 shrink-0 opacity-80" />
              ...
              >
                <History className="size-5 shrink-0 opacity-80" />
              ...
              >
                <Keyboard className="size-5 shrink-0 opacity-80" />
              ...
              {canEditContent && (
                ...
                >
                  <ArchiveRestore className="size-5 shrink-0 opacity-80" />
              ...
              {canEditContent && (
                ...
                >
                  <LayoutGrid className="size-5 shrink-0 opacity-80" />
              ...
              {canDelete && (
                <Button
                  type="button"
                  variant="outline"
                  className="h-auto w-full justify-start gap-3 border-danger/30 bg-surface-50 py-4 text-lg font-semibold text-danger hover:bg-danger/10 dark:border-danger/40 dark:bg-surface-900 dark:hover:bg-danger/15"
                  onClick={handleOpenDeleteDialog}
                >
                  <Trash2 className="size-5 shrink-0 opacity-90" />
          ...
          {panel === "archivedTasks" && (
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
              <div className="shrink-0 border-b border-surface-200 bg-surface-100 p-4 dark:border-surface-800 dark:bg-surface-950">
                <div className="relative">
                  <Search
                    className="text-muted-foreground pointer-events-none absolute top-1/2 left-2.5 size-5 -translate-y-1/2"
                    aria-hidden
                  />
                  <Input
                    value={archivedQuery}
              ...
              {!archivedLoading && (
                <ul className="min-h-0 flex-1 space-y-3 overflow-y-auto bg-surface-100 p-4 text-base dark:bg-surface-950">
                  ...
                  {filteredArchivedTasks.map((taskRow) => {
                      const busy = archivedRowBusy === taskRow._id;
                      let archivedLabel = "—";
                      if (taskRow.archivedAt) {
                        archivedLabel = new Date(taskRow.archivedAt).toLocaleDateString(
                          "es-ES",
                          {
                            day: "2-digit",
                            month: "short",
                          ...
                          <div className="mt-3 flex flex-wrap gap-2">
                            {canEditContent && (
                              <Button
                                ...
                                onClick={() =>
                                  void handleRestoreArchivedTask(taskRow._id)
                                }
                              >
                                {!busy && "Restaurar"}
                            ...
                            {canSettings && (
                              <Button
                                ...
                                disabled={busy || archivedPurgeBusy}
                                onClick={() => setArchivedPurgeTarget(taskRow)}
                              >
          ...
          {panel === "activity" && (
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
              <div className="shrink-0 border-b border-surface-200 bg-surface-100 p-4 text-base text-muted-foreground dark:border-surface-800 dark:bg-surface-950">
                ...
                <ul className="min-h-0 flex-1 space-y-3 overflow-y-auto bg-surface-100 p-4 text-base dark:bg-surface-950">
                  ...
                  {boardActivityLogs.length > 0 &&
                    boardActivityLogs.map((entry) => {
                      ...
                      let createdLabel = entry.createdAt;
                      if (!Number.isNaN(date.getTime())) {
                        createdLabel = date.toLocaleString("es-ES", {
                          day: "2-digit",
                          month: "short",
                          year: "numeric",
                          hour: "2-digit",
          ...
          {panel === "archivedColumns" && (
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-surface-100 p-4 text-base dark:bg-surface-950">
              ...
              <ul className="min-h-0 flex-1 space-y-3 overflow-y-auto">
                {(board.archivedColumns ?? []).length === 0 && (
                  <li className="text-muted-foreground rounded-lg border border-surface-200 bg-surface-100/70 px-4 py-6 text-center text-sm dark:border-surface-700 dark:bg-surface-950/40">
                    No hay columnas archivadas.
                  </li>
                )}
                {(board.archivedColumns ?? []).map((archivedColumn) => {
                    const busy = columnRowBusy === archivedColumn._id;
                    let archivedLabel = "—";
                    if (archivedColumn.archivedAt) {
                      archivedLabel = new Date(archivedColumn.archivedAt).toLocaleDateString(
                        "es-ES",
                        {
                          day: "2-digit",
                          month: "short",
                    ...
                    return (
                      <li
                        key={archivedColumn._id}
                        className="rounded-lg border border-surface-200 bg-surface-100/80 p-3 dark:border-surface-700 dark:bg-surface-950/40"
                      >
                        <p className="truncate font-medium text-surface-900 dark:text-surface-50">
                          {archivedColumn.title}
                        </p>
                        <p className="text-muted-foreground mt-1 text-xs">
                          Archivada: {archivedLabel}
                        </p>
                        <div className="mt-3 flex flex-wrap gap-2">
                          {canEditContent && (
                            <Button
                              ...
                              onClick={() =>
                                void handleRestoreArchivedBoardColumn(archivedColumn._id)
                              }
                            >
                              {!busy && "Restaurar"}
                              {busy && <Loader2 className="size-3.5 animate-spin" />}
                          ...
                          {canSettings && (
                            <Button
                              ...
                              className="border-danger/40 text-danger hover:bg-danger/10 dark:hover:bg-danger/15"
                              disabled={busy || columnPurgeBusy}
                              onClick={() => setColumnPurgeTarget(archivedColumn)}
                            >
                              Borrar definitivamente
                            </Button>
                          )}
          ...
          {panel === "sprints" && (
            <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto bg-surface-100 p-4 text-base sm:p-6 dark:bg-surface-950">
              {sprintsPanelError && (
                <p className="rounded-md border border-danger/30 bg-danger/10 px-3 py-2 text-sm text-danger">
                  {sprintsPanelError}
                </p>
              ...
              <div className="rounded-lg border border-surface-200 bg-surface-100/80 p-4 dark:border-surface-700 dark:bg-surface-950/40">
                <div className="mb-3 flex items-center gap-2 text-base text-surface-800 dark:text-surface-100">
                  <span>Habilita sprints para usar el flujo de sprint en este tablero.</span>
                  <Tooltip delayDuration={250}>
                    <TooltipTrigger asChild>
                      <button
                        type="button"
                        className="inline-flex shrink-0 rounded-sm p-1 text-muted-foreground hover:bg-surface-200/80 hover:text-surface-700 dark:hover:bg-surface-700/80 dark:hover:text-surface-200"
                        aria-label="Más información sobre sprints"
                      >
                        <Info className="size-5" aria-hidden />
                      </button>
                    </TooltipTrigger>
                    <TooltipContent side="right" className="max-w-xs text-base leading-snug">
                ...
                <label className="flex cursor-pointer items-start gap-3">
                  ...
                  />
                  <span className="text-base leading-snug text-surface-800 dark:text-surface-100">
              ...
              {board.activeSprintId && (
                <div className="rounded-lg border border-surface-200 bg-surface-100/80 p-4 dark:border-surface-700 dark:bg-surface-950/40">
                  <h3 className="mb-3 text-lg font-semibold text-surface-800 dark:text-surface-200">
                    Sprint activo
                  </h3>
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Textarea
                        ...
                        maxLength={2000}
                        rows={3}
                        className="min-h-18 resize-y text-base"
                      />
                    </div>
                    <Button
                      type="button"
                    ...
                    >
                      ...
                      {!activeSprintSaveBusy && "Guardar cambios del sprint"}
                    </Button>
                  </div>
                </div>
              )}
              ...
              <div className="min-h-0">
                ...
                {(board.closedSprintRecords ?? []).length === 0 && (
                ...
                {(board.closedSprintRecords ?? []).length > 0 && (
                  <ul className="mt-2 space-y-3 pr-1">
                    {(board.closedSprintRecords ?? []).slice().reverse().map((record) => {
          ...
          {panel === "shortcuts" && (
            <div className="flex min-h-0 flex-1 flex-col overflow-y-auto bg-surface-100 p-4 text-base sm:p-6 dark:bg-surface-950">
                ...
                {KEYBOARD_SHORTCUTS.map((shortcut) => (
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <span className="rounded-md border border-surface-300 bg-surface-50 px-2.5 py-1 text-sm font-semibold tracking-wide text-surface-700 dark:border-surface-600 dark:bg-surface-900 dark:text-surface-200">
                        {shortcut.keys}
                      </span>
                    ...
                    <p className="mt-2 text-base leading-snug text-surface-800 dark:text-surface-100">
                      {shortcut.action}
                    </p>
                  </li>
                ))}
          ...
          {panel === "edit" && (
            <div className="flex flex-1 flex-col gap-5 overflow-y-auto bg-surface-100 p-4 text-base sm:p-6 dark:bg-surface-950">
              ...
              <SheetFooter className="mt-auto flex-row justify-end gap-2 border-t border-surface-200 p-0 pt-4 sm:justify-end dark:border-surface-800">
                ...
                >
                  {savingBoard && <Loader2 className="size-4 animate-spin" />}
                  {!savingBoard && "Guardar"}
                </Button>
              </SheetFooter>
            </div>
          ...
          {panel === "members" && (
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
              ...
              {canManageParticipantsUI && boardDocId && (
                <div className="shrink-0 border-b border-surface-200 bg-surface-100/60 p-4 sm:p-6 dark:border-surface-800 dark:bg-surface-950/40">
                  <BoardInviteBlock
              ...
              <div className="shrink-0 border-b border-surface-200 bg-surface-100 px-4 pt-4 pb-2 text-lg font-semibold text-surface-800 dark:border-surface-800 dark:bg-surface-950 dark:text-surface-200 sm:px-6">
                En el tablero
              </div>
              {membersLoading && (
                <div className="flex flex-1 items-center justify-center p-8">
              ...
              {!membersLoading && (
                <ul className="min-h-0 flex-1 space-y-4 overflow-y-auto bg-surface-100 p-4 pt-2 sm:p-6 sm:pt-3 dark:bg-surface-950">
                  {members.map((member) => {
                    const isOwnerRow =
                      ...
                      >
                        <div className="flex gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            {!isOwnerRow && showManage && (
                              <>
                                <div className="mt-3 mb-2">
                                  <Label>Rol</Label>
                                  <select
                                    className="border-input bg-background mt-1 h-11 w-full rounded-lg border px-2 text-base outline-none focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/50 dark:bg-input/30"
                                    value={draft}
                                    disabled={busy}
                                    onChange={(event) =>
                                      setRoleDraft((draftMap) => ({
                                        ...draftMap,
      ...
      >
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            >
              {expelling && <Loader2 className="size-4 animate-spin" />}
              {!expelling && "Expulsar"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            >
              ...
              {!deleteClosedBusy && "Eliminar del historial"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            >
              ...
              {!columnPurgeBusy && "Borrar permanentemente"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            >
              ...
              {!archivedPurgeBusy && "Borrar permanentemente"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
